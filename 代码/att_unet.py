...
from tensorflow.keras import backend, layers, regularizers
...
class Att_U_Net():
    def __init__(self):
        ...
        # u_net
        self.attunet = self.build_attunet()  # 创建网络变量
        self.attunet.compile(loss='mse',  # 损失函数:均方误差（L2 loss）或者'mean_squared_error'
                          optimizer=optimizer, # 优化器
                          metrics=[self.metric_fun]) # 评价指标
        self.attunet.summary()

    def build_attunet(self, n_filters=16, dropout=0.1, batchnorm=True, padding='same'):
    # n_filters是卷积核数量，dropout是Dropout层的参数，batchnorm控制是否进行标准化，padding控制卷积前后图片尺寸是否变化
        ...
        # extending path
        u6 = Conv2DTranspose(n_filters * 8, (3, 3), strides=(2, 2), padding='same')(c5) # 反卷积（上采样）
        att6= cbam_block(c4) # 注意力块
        u6 = concatenate([u6, att6]) # 连接
        u6 = Dropout(dropout)(u6)
        c6 = conv2d_block(u6, n_filters=n_filters * 8, kernel_size=3, batchnorm=batchnorm, padding=padding)

        u7 = Conv2DTranspose(n_filters * 4, (3, 3), strides=(2, 2), padding='same')(c6)
        att7= cbam_block(c3) # 注意力块
    ...
    def train(self, epochs=101, batch_size=10):
        ...
        # 加载已经训练的模型
        # self.attunet.load_weights(r"weights/best_model_att.h5")

        # 设置训练的checkpoint
        callbacks = [EarlyStopping(patience=100, verbose=2),
                     ReduceLROnPlateau(factor=0.5, patience=20, min_lr=0.00005, verbose=2),
                     ModelCheckpoint('./weights/best_model_att.h5', verbose=2, save_best_only=True)]
        ...
        # 进行训练
        results = self.attunet.fit(x_source, x_label, batch_size=batch_size, epochs=epochs, verbose=2,
        ...
        fig.savefig('./evaluation/curve_att.png', bbox_inches='tight', pad_inches=0.1)  # 保存绘制曲线的图片
        plt.close()

    def test(self, batch_size=1):
        # 将输出结果写入unet_dice.txt文件
        file_handle=open('./evaluation/unet_att_dice.txt',mode='a+',encoding='utf-8')
        file_handle.write('dice of test data is:\n')
        os.makedirs('./evaluation/test_result_att', exist_ok=True)
        self.attunet.load_weights(r"weights/best_model_att.h5")
        # 获得数据
        x_source, x_label, y_source, y_label = self.load_data()
        test_num = y_source.shape[0]
        index, step = 0, 0
        self.attunet.evaluate(y_source, y_label)
        n = 0.0
        dice_list = []
        while index < test_num:
            print('schedule: %d/%d' % (index, test_num))
            step += 1  # 记录训练批数
            mask = self.attunet.predict(y_source[index:index + batch_size]) > 0.1
            ...
            mask.save('./evaluation/test_result_att/' + str(step) + '.png')
            mask_true = Image.fromarray(np.uint8(mask_true * 255))
            mask_true.save('./evaluation/test_result_att/' + str(step) + 'true.png')
            index += batch_size
            gc.collect()
        acc = n / test_num * 100
    ...
    def test1(self, batch_size=1):
        ...
        for epoch in range(5):
            ...
            for i, index in enumerate(rand_index):
                mask = self.attunet.predict(y_source[index:index + 1]) > 0.1
            ...
            fig.savefig('./evaluation/att_show%d_%d_%d.png' % (rand_index[0], rand_index[1], rand_index[2]),
...
if __name__ == '__main__':
    Att_unet = Att_U_Net()
    # Att_unet.train()    # 开始训练网络
    # Att_unet.test()     # 评价测试集并检测测试集肿瘤分割结果
    Att_unet.test1()  # 随机显示
